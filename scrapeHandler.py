...
#-------------------------------INJECT-COOKIES-------------------------------
    ...
    time.sleep(1)
    #com(link[:-23])
    user_name = driver.find_element(By.CSS_SELECTOR, '.text-heading-xlarge.inline.t-24.v-align-middle.break-words').text
    pp_url  = driver.find_element(By.CSS_SELECTOR,'.pv-top-card-profile-picture__image.pv-top-card-profile-picture__image--show.ember-view').get_attribute('src')
    followers = driver.find_element(By.CSS_SELECTOR, '.text-body-small.t-black--light.inline-block').text[:-9]
    ...
    time.sleep(5)
    driver.find_element(By.CSS_SELECTOR,'.artdeco-pill-choice-group.ember-view').find_elements(By.TAG_NAME,'button')[2].click() #driver.get(link)
    time.sleep(5)
    #com(link[:-23])
    ...
    while True:
        ...
        #button = driver.find_element(By.CSS_SELECTOR,'.artdeco-button.artdeco-button--muted.artdeco-button--1.artdeco-button--full.artdeco-button--secondary.ember-view.scaffold-finite-scroll__load-button')
        ...
        if re.search(r'\b3mo\b', check_yr) or re.search(r'\b4mo\b', check_yr) or re.search(r'\b6mo\b', check_yr):
            ...
            com('Len of post push'+str(len(posts_itt)))
            driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
            driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
            time.sleep(5)

            posts = driver.find_elements(By.CSS_SELECTOR, '.ember-view.occludable-update')[:-1]
